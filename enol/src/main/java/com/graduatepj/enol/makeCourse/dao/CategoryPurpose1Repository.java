...
public interface CategoryPurpose1Repository extends JpaRepository<CategoryPurpose1, String> {
...
//    List<Integer> findWalkDrinkExperienceHealingWatchIntellectualViewNormalSportsSoloByCategoryGroupCode(@Param("categoryGroupCode") String categoryGroupCode);
    ...
    @Query(value = "select * from Category_Purpose1 cp where cp.category_group_code=:categoryGroupCode AND ((:walk=1 AND cp.walk=1) OR (:drink=1 AND cp.drink=1) OR (:healing=1 AND cp.healing=1) OR (:socializing=1 AND cp.socializing=1) OR (:mood=1 AND cp.mood=1) OR (:novelty=1 AND cp.novelty=1) OR (:daily=1 AND cp.daily=1) OR (:quick=1 AND cp.quick=1) OR (:summer=1 AND cp.summer=1) OR (:cultural=1 AND cp.cultural=1))", nativeQuery = true)
    List<CategoryPurpose1> findByCategoryGroupCode(@Param("categoryGroupCode") String categoryGroupCode, @Param("walk") Integer walk, @Param("drink") Integer drink, @Param("healing") Integer healing, @Param("socializing") Integer socializing, @Param("mood") Integer mood, @Param("novelty") Integer novelty, @Param("daily") Integer daily, @Param("quick") Integer quick, @Param("summer") Integer summer, @Param("cultural") Integer cultural);
